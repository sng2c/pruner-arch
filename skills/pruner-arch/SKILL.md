...
*   **Test** every black box of this layer (and any inherited from above): *open* it (set $I_{n+1} =$ that black box) and re-run Stages 1–3 on it.
    ...
    *   An a-priori antinomy flagged at Stage 1 is a resist signal *only when paired* with this empirical test (§4 "No transcendental shortcut").
...
When the fixed point is reached — no remaining black box can be reduced to $y=f(x)$; every surviving control resisted (empirically idempotent or a-priori regulative-by-structure) — emit the **Final Architecture Report** below, then await confirmation.

```text
---
**[Pruner] Final Architecture Report — fixed point at layer N**

1. Fixed point reached. No remaining black box can be reduced to deterministic functional mappings y=f(x); every surviving control resisted — empirically (re-opening idempotent) or a priori (antinomy: regulative-by-structure). Ideals flagged regulative-by-structure: [list, with thesis/antithesis]; these are held as regulative limits, not constitutively realizable. Surviving controls under them resisted empirically (idempotent).
...
2. Discovered control panel (the practical mechanism):
   - Real Contextual Intelligence Controls (judgment & responsibility gates):
     ...
     - …
   - Deterministic Core (true y=f(x), a priori, accumulated across layers; same input ⇒ same output): …
   - Semi-Automated Approximations (f̂(x), a posteriori, learned/probabilistic — NOT deterministic; revisable, own drift risk): …
...
5. Review gate. The AI's dissolve/resist verdicts (empirical and a priori) are provisional. Confirm the fixed point here, or re-open any control where a deterministic f(x) (or a semi-automated f̂(x)) was missed, an antinomy was mis-detected, or an intelligence gate was prematurely dissolved.
